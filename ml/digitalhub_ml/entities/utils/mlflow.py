...
from digitalhub_ml.entities.model.spec import Dataset, Signature

def from_mlflow_run(run_id):
    ...
    signature = Signature(
        ...
        params=mlflow_signature.params.to_json() if mlflow_signature.params else None).dict()
    datasets = [] 
    if run.inputs and run.inputs.dataset_inputs:
        datasets = [Dataset(name=d.dataset.name, 
                            digest=d.dataset.digest, 
                            profile=d.dataset.profile, 
                            schema=d.dataset.schema, 
                            source=d.dataset.source, 
                            source_type=d.dataset.source_type).dict() for d in run.inputs.dataset_inputs]
