...
@pytest.fixture
def s3_store():
    config = S3StoreConfig(bucket_name="test_bucket", endpoint_url="http://localhost:9000", aws_access_key_id="test", aws_secret_access_key="test")
...
@patch("digitalhub_core.stores.objects.s3.S3Store._download_file")
...
@patch("digitalhub_core.stores.objects.s3.S3Store._upload_file")
