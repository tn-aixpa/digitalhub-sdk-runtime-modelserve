...
class Service(Entity):
    ...
    def save(self, update: bool = False) -> dict:
        ...
        api = api_ctx_update(self.project, "services", self.name, self.id)
    ...
    def _parse_dict(
        ...
        status = build_status(ServiceStatus, **obj.get("status", {}))
        return {
            "project": project,
            "name": name,
            "uuid": uuid,
...
def service_from_parameters(
    ...
    **kwargs
    ...
    status = build_status(ServiceStatus)
    return Service(
        project=project,
        name=name,
        uuid=uuid,
...
def service_from_dict(obj: dict) -> Service:
    ...
    ----------
    obj : dict
        Dictionary to create Service from.
